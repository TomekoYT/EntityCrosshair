...
object EntityDrawer : BasicOption(null, null, "", "", "", "", 2) {
    ...
    init {
        toBufferedImage(EntityCrosshairConfig.entityCanvaConfig.newCurrentCrosshair.img)?.let { it ->
            if (it.width == 0 || it.height == 0) return@let
            loadImage(it, false, EntityCrosshairConfig.entityCanvaConfig.newCurrentCrosshair)?.let {
                CrosshairRenderer.updateEntityTexture(it)
            }
        }
        ...
        saveButton.setClickAction {
            runAsync {
                saveEntity(saveFromDrawer(false))
            }
        }
    ...
    override fun draw(vg: Long, x: Int, y: Int, inputHandler: InputHandler) {
        for (posY in 0..<EntityCrosshairConfig.entityCanvaConfig.canvaSize) {
            for (posX in 0..<EntityCrosshairConfig.entityCanvaConfig.canvaSize) {
                pixels[posToIndex(posX, posY)].draw(vg, x.toFloat(), y.toFloat(), inputHandler)
            }
        }

        if (EntityCrosshairConfig.entityCanvaConfig.canvaSize % 2 == 0) {
        ...
        val height = (149 + 16) * ceil(EntityCrosshairConfig.entityCanvaConfig.newCrosshairs.size / 4f) - 16
        ...
        val size = EntityCrosshairConfig.entityCanvaConfig.newCrosshairs.size

        for (i in 0..<size) {
            val posX = i % 4
            val posY = i / 4
            getElement(EntityCrosshairConfig.entityCanvaConfig.newCrosshairs[i]).draw(
                vg,
                x + 349 + posX * 165f,
                y + posY * 165f + scroll,
                inputHandler
    ...
    fun loadImage(image: BufferedImage?, save: Boolean, entry: EntityCrosshairEntry = EntityCrosshairEntry()): OneImage? {
        ...
        EntityCrosshairConfig.entityCanvaConfig.newCurrentCrosshair.loadFrom(entry)
        EntityCrosshairConfig.entityCanvaConfig.canvaSize = loadedImage.height
        for (posY in 0..<EntityCrosshairConfig.entityCanvaConfig.canvaSize) {
            for (posX in 0..<EntityCrosshairConfig.entityCanvaConfig.canvaSize) {
        ...
        if (save) saveEntity(loadedImage)
        return loadedImage
    }

    fun saveFromDrawer(close: Boolean): OneImage? {
        val image = OneImage(EntityCrosshairConfig.entityCanvaConfig.canvaSize, EntityCrosshairConfig.entityCanvaConfig.canvaSize)
        if (EntityCrosshairConfig.entityCanvaConfig.drawerMap.isEmpty() && !close) {
            notify("Crosshair can't be empty.")
            return null
        }
        for (i in EntityCrosshairConfig.entityCanvaConfig.drawerMap) {
            val pos = indexToPos(i.key)
            if (pos.x >= EntityCrosshairConfig.entityCanvaConfig.canvaSize || pos.y >= EntityCrosshairConfig.entityCanvaConfig.canvaSize) {
    ...
    override fun finishUpAndClose() {
        ...
        EntityCrosshairConfig.entityCanvaConfig.newCurrentCrosshair.img = toBase64(image.image)
        CrosshairRenderer.updateEntityTexture(image)
    }
