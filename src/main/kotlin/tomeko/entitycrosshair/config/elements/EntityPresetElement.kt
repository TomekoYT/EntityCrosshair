...
class EntityPresetElement(val crosshair: EntityCrosshairEntry) : BasicElement(149, 149, ColorPalette.SECONDARY, true) {
    ...
    init {
        removeButton.setClickAction {
            EntityDrawer.removeQueue.add(crosshair)
        }
    ...
    override fun update(x: Float, y: Float, inputHandler: InputHandler) {
        hovered = EntityDrawer.inArea && inputHandler.isAreaHovered(x - hitBoxX, y - hitBoxY, (width + hitBoxX).toFloat(), (height + hitBoxY).toFloat())
    ...
    fun onRemove() {
        ...
        EntityDrawer.elements.remove(crosshair)
    }
    ...
    override fun onClick() {
        ...
        EntityDrawer.clear()
        EntityDrawer.loadImage(bufferedImage, false, crosshair)
    }
}